...
export function ProfileForm() {
  ...
  return <form onSubmit={save} className="flex flex-wrap items-end gap-2"><label className="text-sm">Display name<input disabled={!loaded || saving} required maxLength={50} value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="mt-1 block rounded border border-slate-300 px-3 py-2" /></label><button disabled={!loaded || saving} className="rounded border border-slate-300 px-3 py-2 text-sm disabled:opacity-50">Save</button>{message && <p role="status" className="text-sm text-slate-600">{message}</p>}</form>;
}
